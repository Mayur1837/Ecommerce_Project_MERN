...
export const getProduct =
  ...
  async (dispatch) => {
    //keyword="" means if nothing comes then empty
    try {
...
export const updateProduct = (id, productData) => async (dispatch) => {
  try {
    ...
    // console.log("Error occured during  product updating");
    dispatch({
...
export const deleteProduct = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.delete(`/api/v1/products/deleteproduct/${id}`);
...
export const getAllReviews = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`/api/v1/getallreviews?id=${id}`);
